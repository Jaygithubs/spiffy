<div class="header-mobile">
    <div class="section-padding">
        <div class="section-container">
            <div class="row">
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-3 col-3 header-left">
                    <div class="navbar-header">
                        <button type="button" id="show-megamenu" class="navbar-toggle"></button>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-6 header-center">
                    <div class="site-logo">
                        <a href="index.php">
                            <img width="400" height="79" src="media/logo1.png" alt="Spiffy Sole" />
                        </a>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-3 col-3 header-right">
                    <div class="ruper-topcart dropdown">
                        <div class="dropdown mini-cart top-cart">
                            <a href="shop-checkout.html" class="button btn checkout btn-default">Order Now</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="header-mobile-fixed">
        <!-- Shop -->
        <div class="shop-page">
            <a href="shop-grid-left.html"><i class="wpb-icon-shop"></i></a>
        </div>

        <!-- Login -->
        <div class="my-account">
            <div class="login-header">
                <a href="page-my-account.html"><i class="wpb-icon-user"></i></a>
            </div>
        </div>

        <!-- Search -->
        <div class="search-box">
            <div class="search-toggle"><i class="wpb-icon-magnifying-glass"></i></div>
        </div>

        <!-- Wishlist -->
        <div class="wishlist-box">
            <a href="shop-wishlist.html"><i class="wpb-icon-heart"></i></a>
        </div>
    </div>
</div>

<div class="header-desktop">
    <div class="header-wrapper">
        <div class="section-padding">
            <div class="section-container p-l-r">
                <div class="row">
                    <div class="col-xl-3 col-lg-2 col-md-12 col-sm-12 col-12 header-left">
                        <div class="site-logo">
                            <a href="index.php">
                                <img width="400" height="79" src="media/white and yellow logo.png" alt="Spiffy Sole" />
                            </a>
                        </div>
                    </div>

                    <div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12 text-center header-right">
                        <div class="site-navigation">
                            <nav id="main-navigation">
                                <ul id="menu-main-menu" class="menu">
                                    <li class="level-0 menu-item">
                                        <a href="index.php" style="color: #fff;"><span class="menu-item-text">Home</span></a>
                                    </li>
                                    <li class="level-0 menu-item">
                                        <a href="about.php" style="color: #fff;"><span class="menu-item-text">Our Story</span></a>
                                    </li>
                                    <li class="level-0 menu-item menu-item-has-children">
                                        <a href="" style="color: #fff;"><span class="menu-item-text">Categories</span></a>
                                        <ul class="sub-menu">
                                            <li>
                                                <a href="shop.php"><span class="menu-item-text">Shoes</span></a>
                                            </li>
                                            <li>
                                                <a href="shop.php"><span class="menu-item-text">Flats</span></a>
                                            </li>
                                            <li>
                                                <a href="shop.php"><span class="menu-item-text">Demin Jeans</span></a>
                                            </li>
                                            <li>
                                                <a href="shop.php"><span class="menu-item-text">Demin Jackets</span></a>
                                            </li>
                                        </ul>
                                    </li>
                                    <li class="level-0 menu-item">
                                        <a href="contact.php" style="color: #fff;"><span class="menu-item-text">Contact</span></a>
                                    </li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>